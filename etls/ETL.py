...
def seleccionar_columnas(df_name,nombre_columnas=[]):
    ...
    df_limpio=df_name[nombre_columnas].copy()
...
def limpiar_espacios_en_una_columna(df_name,columna):
    ...
    try:
        df_name[columna]=df_name[columna].str.strip()
    except:
        pass
...
def limpiar_caracteres(df_name,columna,char=[]):
    ...
    for y in char:
        try:
            df_name[columna]=df_name[columna].str.replace(y,"")
        except:
            pass
...
def crear_df_inventario():
    ...
    dataframe=lectura_tablas(ruta_materia)
...
def main():
    ...
    df_materia=crear_df_inventario()
    df_notven_inventario=merge_dataframes(df_notven,df_materia)
    cat_meseros=crear_cat_meseros()
    tabla_maestra=crear_tabla_maestra(df_notven_inventario,cat_meseros)
    tabla_maestra.to_csv("tabla_maestra.csv")
    # tabla_maestra_2015=tabla_maestra.query('anio_vendido==2015')
    # tabla_maestra_2016=tabla_maestra.query('anio_vendido==2016')
    # tabla_maestra_2017=tabla_maestra.query('anio_vendido==2017')
    # tabla_maestra_2018=tabla_maestra.query('anio_vendido==2018')
    # tabla_maestra_2019=tabla_maestra.query('anio_vendido==2019')
    # tabla_maestra_2020=tabla_maestra.query('anio_vendido==2020')
    # tabla_maestra_2015.to_csv("tabla_maestra_2015.csv")
    # tabla_maestra_2016.to_csv("tabla_maestra_2016.csv")
    # tabla_maestra_2017.to_csv("tabla_maestra_2017.csv")
    # tabla_maestra_2018.to_csv("tabla_maestra_2018.csv")
    # tabla_maestra_2019.to_csv("tabla_maestra_2019.csv")
    # tabla_maestra_2020.to_csv("tabla_maestra_2020.csv")

    # engine=conexion_SQL(ppt.usuario,ppt.contrasena,ppt.puerto,ppt.base_datos)    
    # tabla_maestra.to_sql('tbl_maestra_prueba',engine,if_exists='replace')
    # df_notven.to_sql('tabla_notven',engine,if_exists='replace',index=False)
    # df_materia.to_sql('tabla_materia',engine,if_exists='replace',index=False)
    # cat_meseros.to_sql('cat_meseros',engine,if_exists='replace',index=False)
